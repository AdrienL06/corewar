 ...
 * @details This function executes the ADD instruction, which adds the values of
 *          two arguments and stores the result in a third argument. The
 *          function performs validity checks on the registers specified in the
 *          arguments, retrieves their values, performs the addition, and stores
 *          the result in the destination register. The carry flag of the
...
int add_run(arena_t *arena, pfork_t *player, instruction_arg_t *args)
{
    ...
    set_argument_value(&args[2], player, arena, add_value);
